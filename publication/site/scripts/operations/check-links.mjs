...
function ignored(pathname) {
  return (
    pathname.startsWith('/social-card') || /^\/editions\/[^/]+\/(?:print|epub)$/.test(pathname)
  );
}
  ...
  for (const name of ['navigation.json', 'documents.json']) {
    try {
      const value = JSON.parse(
        await readFile(resolve(siteRoot, '.generated', 'registry', name), 'utf8'),
      );
...
await withProductionServer(async (baseUrl) => {
  ...
  while (queue.length > 0 && visited.size < 1500) {
    ...
    if (response.status >= 400) {
      failures.push({ route: pathname, status: response.status });
      continue;
    }
  ...
  if (failures.length > 0) {
    for (const failure of failures)
      console.error(`BROKEN ${failure.route}: ${failure.status ?? failure.error}`);
    process.exitCode = 1;
  }
});
