...
* backend-neutral, target-oriented KIR structure;
...
KIR lowering is the governed boundary that selects an eligible semantic subgraph and transforms it into a backend-neutral operational contract scoped to one declared target or compatible target family.
...
**Knowledge Intermediate Representation.** A backend-neutral, target-oriented operational intermediate representation derived from an eligible MSG subgraph.
...
A KIR module groups backend-neutral operational units scoped to one declared target or compatible target family.
...
KIR types are backend-neutral operational types interpreted within the KIR snapshot’s declared target or compatible target family.
...
* identity uniqueness;
* schema conformance;
* backend-neutral type validity within the declared target scope;
* dependency closure;
...
A KIR backend is a governed consumer of validated KIR.
...
1. Which compatibility rules permit KIR reuse across targets within one declared target family?
...
KIR backends operate only on an explicit, validated, backend-neutral contract scoped to a declared target or target family. Generated artifacts remain projections with complete identity, ownership, provenance, and lifecycle. Mutation is planned, authorized, verified, and recoverable.
...
This amendment is normative and supersedes any earlier unqualified use of `target-independent` or untyped `backend` within this artifact.

### Canonical KIR Definition

**KIR** means **Knowledge Intermediate Representation**.

KIR is a backend-neutral, target-oriented operational projection derived from an eligible MSG subgraph. One KIR snapshot declares exactly one target or one explicitly compatible target family. Several KIR backends may consume the same KIR snapshot when their manifests satisfy that target contract.

A target change requires a distinct KIR snapshot whenever eligibility, types, operations, resources, constraints, effects, or verification obligations differ.

### Consumer Taxonomy

```text
CompilerPass   — consumes and produces compiler representations or analysis
MSGConsumer    — consumes MSG or persisted MSG knowledge for ingestion or projection
KIRBackend     — consumes validated KIR and produces governed plans or outputs
ExternalTool   — consumes declared inputs under a tool contract
```

A `KIRBackend` must not read raw source or arbitrary MSG to invent semantics absent from KIR. Publication, AI-context, graph-query, and MKE-ingestion components are `MSGConsumer` implementations unless they explicitly consume KIR.

### KIR and Concrete Generation Plans

KIR owns abstract operational intent, declarations, types, operations, resources, effects, constraints, validations, and backend requirements.

The KIR backend owns the concrete `GeneratedArtifactPlan`, including destination paths, file operations, tool invocations, overwrite decisions, rollback operations, and target-specific verification steps.

### Optimization and Backend IR

A KIR optimization is a governed, fingerprinted, provenance-preserving KIR transformation.

Backend-internal IR is noncanonical, backend-owned, and derived from KIR. It participates in backend cache and reproducibility contracts when it can alter output, but it never becomes canonical semantic authority.

### Bootstrap Decision Gates

| Decision | Required before |
| --- | --- |
| Canonical AST bootstrap schema | canonical AST implementation |
| MSG bootstrap schema and serialization | MSG construction implementation |
| KIR bootstrap schema and serialization | KIR lowering implementation |
| Minimum KIR types and operations | first KIR backend |
| Canonical encoding and fingerprint algorithm | reproducibility verification |
| Diagnostic JSON schema | stable machine diagnostics |
| Source-map representation | cross-representation diagnostics |
| First generated compiler artifact | Stage 2 self-hosting |

The Stage 0 implementation language, internal data structures, concurrency runtime, cache library, CLI framework, and test framework remain implementation-work-packet choices unless a later accepted ADR governs them.

### Additional Requirements

* A KIR snapshot **MUST** identify its target or compatible target family.
* KIR **MUST NOT** become canonical semantic authority.
* A KIR backend **MUST** consume validated KIR.
* Concrete generation plans **MUST** be backend outputs.
* A candidate compiler **MUST NOT** promote itself.
* Self-hosting promotion **MUST** preserve bootstrap, candidate, divergence, security, reproducibility, approval, and rollback evidence.
