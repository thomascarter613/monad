...
MSG construction consumes an immutable or completely fingerprinted `SemanticGraphConstructionInput` bundle. The bundle references the immutable outputs of preceding phases instead of redefining or reaching backward into mutable compiler state.
...
MSG construction must reject or explicitly degrade bundles whose identity or fingerprint is incomplete. Every referenced snapshot must identify its schema version, compilation-unit identity, active profile, extension set, provenance, and content fingerprint. A bundle containing mutually incompatible snapshots is invalid.
...
machine_spec:
  ...
  input:
    kind: SemanticGraphConstructionInput
...
This amendment is normative for version `0.1.0` and supersedes any earlier wording in this artifact that treats `SemanticAnalysisSnapshot` as an aggregate owner of canonical AST, declaration, symbol, namespace, or reference-resolution state.

### Reconciled Phase Boundary

`MSC-CORE-0007` produces `SemanticAnalysisSnapshot`.

`MSC-CORE-0008` consumes `SemanticGraphConstructionInput`:
...
Every member is immutable or completely fingerprinted. The bundle is invalid when member snapshots disagree about compilation-unit identity, source snapshot, schema compatibility, profile, or extension set.

### Reconciled Output Branches

A valid immutable MSG snapshot has independent downstream branches:

```text
MSG
├──→ MKE ingestion
├──→ KIR lowering
├──→ publication and inspection projections
└──→ AI-context and other semantic projections
```

KIR is not required before MKE ingestion. MKE may persist KIR as derived knowledge without making KIR canonical semantic authority.

### Typed MSG Readiness

`MSGReadiness` is distinct from semantic-analysis, KIR, backend, and generated-artifact readiness. Initial values are:

```text
partial
valid
contextually_authoritative
mke_ingestible
kir_selectable
...
`kir_selectable` means that at least one target-specific selection may be attempted. It does not mean every graph element is KIR-eligible.

### Additional Requirements

* MSC **MUST NOT** obtain graph-construction input from hidden mutable phase state.
* MSC **MUST** validate every referenced snapshot identity and fingerprint before construction.
* MSC **MUST** diagnose profile, schema, extension-set, and compilation-unit mismatches.
* MKE ingestion eligibility **MUST** remain distinct from KIR eligibility.
* Publication eligibility **MUST NOT** elevate authority or imply KIR eligibility.
